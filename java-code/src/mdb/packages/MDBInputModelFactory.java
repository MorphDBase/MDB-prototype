 ...
 * Last modified by Roman Baum on 24.05.17.
 */
...
public class MDBInputModelFactory {

    public MDBInputModelFactory() {
    ...
    public ArrayList<Model> createMDBInputModel(JSONArray triplesJSONArray) {
        ...
        for (int i = 0; i < triplesJSONArray.length(); i++ ) {
            ...
            if (uriValidator.isValid(mdbURLEncoder.encodeUrl(currTripleJSONObject.getString("subject"), "UTF-8"))) {

                currSubject = ResourceFactory.createResource(currTripleJSONObject.getString("subject"));

            } else {

                AnonId anonId = new AnonId(currTripleJSONObject.getString("subject"));
            ...
            if (uriValidator.isValid(mdbURLEncoder.encodeUrl(currTripleJSONObject.getString("property"), "UTF-8"))) {

                currProperty = ResourceFactory.createProperty(currTripleJSONObject.getString("property"));

            } else {

                currProperty = ResourceFactory.createProperty("http://www.dummy.com/property");

                System.out.println("The input value for the property is not valid >>> " + currTripleJSONObject.getString("property"));
            ...
            if (currInputSubObject.getString("object_type").equals("r") &&
                    (   (uriValidator.isValid(mdbURLEncoder.encodeUrl(currInputSubObject.getString("object_data"), "UTF-8"))) ||
                        (EmailValidator.getInstance().isValid(mdbURLEncoder.encodeUrl(currInputSubObject.getString("object_data"), "UTF-8"))))) {
                //System.out.println("The input value is an object.");

                currObject = ResourceFactory.createResource(currInputSubObject.getString("object_data"));

            } else if (currInputSubObject.getString("object_type").equals("l")) {
                //System.out.println("The input value is a literal.");

                if (currInputSubObject.getString("object_data").contains("^^")) {
                    // special case object_data contains a type literal

                    String objectToCheck = currInputSubObject.getString("object_data");
                    ...
                    if (urlValidator.isValid(behindSeparator)) {
                        ...
                        if (XSDDatatype.XSDboolean.getURI().equals(behindSeparator)) {
                            ...
                            currObject = ResourceFactory.createPlainLiteral(currInputSubObject.getString("object_data"));
                ...
                System.out.println("The input value for the object is not valid >>> " + currInputSubObject.getString("object_data"));
            }

            currStatement = ResourceFactory.createStatement(currSubject, currProperty, currObject);

            // differ between save or delete triples
            if (currTripleJSONObject.getString("operation").equals("s")) {

                insertModel.add(currStatement);

            } else if (currTripleJSONObject.getString("operation").equals("d")) {
