 ...
 * Last modified by Roman Baum on 05.01.17.
 */
...
public class OrderDataset {
    ...
    private void sortGenerateResources (JSONArray currExecStep, String pathToTDB, JenaIOTDBFactory connectionToTDB) {
        ...
        for (int i = 0; i < currExecStep.length(); i++) {
            ...
            if (!currExecStep.getJSONObject(i).getString("property").contains("http://www.morphdbase.de/Ontologies/MDB/MDBUserInterfaceAnnotationProperty#MDB_UIAP_0000000233")) {
                ...
                if (urlValidator.isValid(currExecStep.getJSONObject(i).getString("property"))) {

                    tripleSPO.addWhere("<" + currExecStep.getJSONObject(i).getString("property") + ">", "<http://www.morphdbase.de/Ontologies/MDB/MDBUserInterfaceAnnotationProperty#MDB_UIAP_0000000465>", "?o");
                    // has sorting value
                    tripleSPO.addWhere("<" + currExecStep.getJSONObject(i).getString("property") + ">", "<http://www.w3.org/2000/01/rdf-schema#subPropertyOf>", "<http://www.morphdbase.de/Ontologies/MDB/MDBUserInterfaceAnnotationProperty#MDB_UIAP_0000000113>");
                ...
                if (!(selectResult.replaceAll("[\\D]", "").isEmpty())) {
                    ...
                    unorderedObjectsJSONObject.put(selectResult.replaceAll("[\\D]", ""), currExecStep.getJSONObject(i).getString("object"));
                    unorderedPropertiesJSONObject.put(selectResult.replaceAll("[\\D]", ""), currExecStep.getJSONObject(i).getString("property"));
        ...
        for (Integer currIndex : indices) {
            ...
            this.sortedObjects.add(unorderedObjectsJSONObject.getString(String.valueOf(currIndex)));

            this.sortedProperties.add(unorderedPropertiesJSONObject.getString(String.valueOf(currIndex)));
