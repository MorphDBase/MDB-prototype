...
public class MongoDBConnection {
    ...
    public boolean documentWithDataExist(String db, String collectionToCheck, String key, JSONArray valuesInJSON) {
        ...
        for (int i = 0; i < valuesInJSON.length(); i++) {
            ...
            while (keys.hasNext()) {
                ...
                basicDBObject.put(currKey, currObject.getString(currKey));
        ...
        for (int i = 0; i < valuesInJSON.length(); i++) {
            ...
            if (valuesInJSON.get(i).getClass().equals(JSONObject.class)) {
                ...
                while (keys.hasNext()) {
                    ...
                    if (objectValidator.isValid(mdburlEncoder.encodeUrl(currKey, "UTF-8"))) {
                    ...
                    if (currObject.get(currKey) instanceof String) {
                        ...
                        if (currKeyIsURI) {
                            ...
                            basicDBObject.put(localId, currObject.getString(currKey));
                                    ...
                                    while (innerKeys.hasNext()) {
                                        ...
                                        if (currInnerObject.get(currInnerKey) instanceof JSONObject) {
                                            ...
                                            while (innerInnerKeys.hasNext()) {
                                                ...
                                                basicInnerInnerDBObject.put(currInnerInnerKey, currInnerInnerObject.getString(currInnerInnerKey));
                                            ...
                                            basicInnerDBObject.put(currInnerKey, currInnerObject.getString(currInnerKey));
                ...
                valuesInList.add(valuesInJSON.getString(i));
    ...
    public String insertDataToMongoDB (String db, String collection, JSONArray keys, String value) {
        ...
        for (int i = 0; i < keys.length(); i++) {
            ...
            insertDoc.append(keys.getString(i), value);
        ...
        for (int i = 0; i < keys.length(); i++) {
            ...
            if (!documentExist(db, collection, keys.getString(i), value)) {

                return "The document named " + keys.getString(i) + " was not insert in the collection.";
    ...
    public JSONObject pullDataFromMongoDBWithLocalID(JSONObject jsonToFindData) {
        ...
        for (Document doc : mongoCollection.find()) {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                if (jsonObjectFromMongoDB.get("localID").equals(jsonToFindData.getString("localID"))) {
    ...
    public JSONObject pullJSONObjectFromMongoDB(JSONObject jsonToFindData) {
        ...
        // get the collection from the database
        MongoCollection<Document> mongoCollection = database.getCollection(jsonToFindData.getString("html_form"));
    ...
    public JSONArray pullListFromMongoDB(JSONObject jsonToFindData) {
        ...
        for (Document doc : mongoCollection.find()) {
            ...
            return new JSONObject(doc.toJson()).getJSONArray(jsonToFindData.getString("html_form"));
            ...
            if (!mongoDBJSONArray.isNull(0)) {
                ...
                for (int i = 0; i < valuesInJSON.length(); i++) {
                    ...
                    for (int j = 0; j < mongoDBJSONArray.length(); j++) {
                        ...
                        if (mongoDBJSONArray.getJSONObject(j).getString("individualID").equals(valuesInJSON.getJSONObject(i).getString("individualID"))) {
                ...
                for (int i = 0; i < mongoDBJSONArray.length(); i++) {
                    ...
                    if (mongoDBJSONArray.get(i).getClass().equals(JSONObject.class)) {
                        ...
                        valuesInList.add(mongoDBJSONArray.getString(i));
    ...
    public void putJSONInputObjectInMongoDB(JSONObject jsonInputObject) {
        ...
        int count = countCollectionDocuments("mdb-prototyp", jsonInputObject.getString("html_form"));

        if (count > 0) {

            System.out.println("Collection already exist");

            insertDataToMongoDB("mdb-prototyp", jsonInputObject.getString("html_form"), String.valueOf(count), jsonInputArray);

        } else {

            createCollection("mdb-prototyp", jsonInputObject.getString("html_form"));

            insertDataToMongoDB("mdb-prototyp", jsonInputObject.getString("html_form"), "0", jsonInputArray);
