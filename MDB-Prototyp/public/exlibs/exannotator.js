...
(function() {
    ...
    Annotator = (function(_super) {
        ...
        Annotator.prototype.events = {
            ...
            ".annotator-autoadder button click": "onAutoAdderClick"
        };
        ...
        function Annotator(element, options) {
            ...
            this.onAutoAdderClick = __bind(this.onAutoAdderClick, this);
        ...
        Annotator.prototype.dumpAnnotations = function() {
            if (this.plugins['Websocket']) {
                return this.plugins['Websocket'].dumpAnnotations();
            } else {
                console.warn(_t("Can't dump annotations without Websocket plugin."));
                return false;
            }
        };
        ...
        Annotator.prototype.onAdderClick = function(event) {
            ...
            //TODO: hier wird eine annotation angelegt
            annotation = this.setupAnnotation(this.createAnnotation());
            //alert("onAdderClick " + annotation[0]);
        ...
        Annotator.prototype.onAutoAdderClick = function(event) {

            alert("onAutoAdderClick");
            alert("onAutoAdderClick - " + JSON.stringify(event));
            /*var annotation, cancel, cleanup, position, save,
                _this = this;
            if (event != null) {
                event.preventDefault();
    ...
    Annotator.Plugin.Websocket = (function(_super, WsService) {
        __extends(Websocket, _super);

        Websocket.prototype.events = {
            'annotationCreated': 'annotationCreated',
            'annotationDeleted': 'annotationDeleted',
            'annotationUpdated': 'annotationUpdated'
        };

        Websocket.prototype.options = {
            annotationData: {},
            emulateHTTP: false,
            loadFromSearch: false,
            prefix: '/websocket'
        ...
        //alert("WsService - " + WsService.sent);
        //Websocket.prototype.wsService = angular.injector(['ng', 'mdbApp']).get("WsService");

        function Websocket(element, options) {
            this._onError = __bind(this._onError, this);
            this._onLoadAnnotationsFromSearch = __bind(this._onLoadAnnotationsFromSearch, this);
            this._onLoadAnnotations = __bind(this._onLoadAnnotations, this);
            this._getAnnotations = __bind(this._getAnnotations, this);
        ...
        Websocket.prototype.pluginInit = function() {
            if (!Annotator.supported()) {
                return;
            }
            if (this.annotator.plugins.Auth) {
        ...
        Websocket.prototype._getAnnotations = function() {
            if (this.options.loadFromSearch) {
                return this.loadAnnotationsFromSearch(this.options.loadFromSearch);
            } else {
                return this.loadAnnotations();
        ...
        // TODO: _apiRequest
        Websocket.prototype.annotationCreated = function(annotation) {
            var _this = this;
            if (__indexOf.call(this.annotations, annotation) < 0) {
                this.registerAnnotation(annotation);
                ///////////////////////////////////////////
                alert("annotationCreated " + JSON.stringify(annotation));
                // TODO ############################################################################################################
                var anno = [];
                anno.push('create');
                anno.push(annotation);
        ...
        // TODO: _apiRequest
        Websocket.prototype.annotationUpdated = function(annotation) {
            var _this = this;
            if (__indexOf.call(this.annotations, annotation) >= 0) {
                // TODO ############################################################################################################
                var anno = [];
        ...
        // TODO: _apiRequest
        Websocket.prototype.annotationDeleted = function(annotation) {
            var _this = this;
            if (__indexOf.call(this.annotations, annotation) >= 0) {
                // TODO ############################################################################################################
                var anno = [];
        ...
        Websocket.prototype.registerAnnotation = function(annotation) {
            return this.annotations.push(annotation);
        };

        Websocket.prototype.unregisterAnnotation = function(annotation) {
            return this.annotations.splice(this.annotations.indexOf(annotation), 1);
        };

        Websocket.prototype.updateAnnotation = function(annotation, data) {
            if (__indexOf.call(this.annotations, annotation) < 0) {
                console.error(Annotator._t("Trying to update unregistered annotation!"));
            } else {
                $.extend(annotation, data);
        ...
        // TODO: _apiRequest
        Websocket.prototype.loadAnnotations = function() {
            // TODO ############################################################################################################
            var anno = [];
            anno.push('read');
            angular.element(document.querySelector('[ng-app]')).injector().get("WsService").sendAnnotationToWebsocket(anno);
        ...
        Websocket.prototype._onLoadAnnotations = function(data) {
            var a, annotation, annotationMap, newData, _k, _l, _len2, _len3, _ref3;
            if (data == null) {
                data = [];
            }
        ...
        // TODO: _apiRequest
        Websocket.prototype.loadAnnotationsFromSearch = function(searchOptions) {
            // TODO ############################################################################################################
            var anno = [];
            anno.push('search');
            angular.element(document.querySelector('[ng-app]')).injector().get("WsService").sendAnnotationToWebsocket(anno);
        ...
        Websocket.prototype._onLoadAnnotationsFromSearch = function(data) {
            if (data == null) {
                data = {};
            }
            return this._onLoadAnnotations(data.rows || []);
        ...
        Websocket.prototype.dumpAnnotations = function() {
            var ann, _k, _len2, _ref3, _results;
            _ref3 = this.annotations;
            _results = [];
            for (_k = 0, _len2 = _ref3.length; _k < _len2; _k++) {
        ...
        Websocket.prototype._apiRequest = function(action, obj, onSuccess) {
            var id, request;
            id = obj && obj.id;
            url = this._urlFor(action, id);
            ...
            // TODO ///////////////////////////////////////////////////////////////////////////////
            angular.element(document.querySelector('[ng-app]')).injector().get("WsService").sendAnnotationToWebsocket(annotation);


            //angular.injector(['ng', 'mdbApp']).get("WsService").sendAnnotationToWebsocket(options);

            WsService.sendAnnotationToWebsocket("lalalala");
        ...
        Websocket.prototype._apiRequestOptions = function(action, obj, onSuccess) {
            var data, method, opts;
            method = this._methodFor(action);
            opts = {
                type: method,
        ...
        Websocket.prototype._urlFor = function(action, id) {
            var url;
            url = this.options.prefix != null ? this.options.prefix : '';
            url += this.options.urls[action];
            url = url.replace(/\/:id/, id != null ? '/' + id : '');
        ...
        Websocket.prototype._methodFor = function(action) {
            var table;
            table = {
                'create': 'POST',
                'read': 'GET',
        ...
        Websocket.prototype._dataFor = function(annotation) {
            var data, highlights;
            highlights = annotation.highlights;
            delete annotation.highlights;
            $.extend(annotation, this.options.annotationData);
        ...
        Websocket.prototype._onError = function(xhr) {
            var action, message;
            action = xhr._action;
            message = Annotator._t("Sorry we could not ") + action + Annotator._t(" this annotation");
            if (xhr._action === 'search') {
                message = Annotator._t("Sorry we could not search the websocket for annotations");
            } else if (xhr._action === 'read' && !xhr._id) {
                message = Annotator._t("Sorry we could not ") + action + Annotator._t(" the annotations from the websocket");
            }
            switch (xhr.status) {
                case 401:
                    message = Annotator._t("Sorry you are not allowed to ") + action + Annotator._t(" this annotation");
                    break;
                case 404:
                    message = Annotator._t("Sorry we could not connect to the annotations websocket");
                    break;
                case 500:
                    message = Annotator._t("Sorry something went wrong with the annotation websocket");
            }
            Annotator.showNotification(message, Annotator.Notification.ERROR);
            return console.error(Annotator._t("API request failed:") + (" '" + xhr.status + "'"));
        };
    ...
    Annotator.prototype.setupPlugins = function(config, options) {
        ...
        plugins = ['Unsupported', 'Auth', 'Tags', 'Filter', 'Websocket'/*, 'Store'*/, 'AnnotateItPermissions'];
        if (win.Showdown) {
